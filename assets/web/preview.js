...
let isReview = false;
let isPortrait = true;
let mobile_landscope_style;
let isSetScreenSize = false;
let question_label = 'Introduction';
...
setInterval(function () {
    if ($('.screen_height').length > 0 && !isFitted)  {
        ...
        // if ($('.quiz_list_container').eq(quizzes.length) != undefined) {
            //     $('.quiz_list_container').eq(quizzes.length).addClass('quiz_show');
            //     $('.quiz_list_container').eq(quizzes.length).removeClass('quiz_hide');
            // }
        $('#preview_container').append('<div style="position: relative; height: 100vh;"></div>');
...
function hide_some_btns_for_mobile() {
    ...
    $('.question_menu_bar').attr('style', 'margin: 0 15px;display: flex;justify-content: space-between;padding: 20px;border-bottom: 1px solid #e9e9e9;font-size: 20px;');
    $('#question_point').hide();
    $('#question_point').next().hide();
    $('#question_list').attr('style', 'font-weight: bold;');
    $('#question_list').html(question_label);
...
function get_mobile_style() {
    let preview_container_style = $('#preview_container').attr('style');
    let quiz_show_style = $('.quiz_show').attr('style');
    let quiz_show_html = $('.quiz_show').html();
    let preview_toast_style = $('#preview_toast').attr('style');

    return {
        preview_container_style: preview_container_style,
        quiz_show_style: quiz_show_style,
        quiz_show_html: quiz_show_html,
        preview_toast_style: preview_toast_style
    };
}
...
function set_mobile_landscope_style() {
    ...
    $('#preview_toast').attr('style', mobile_landscope_style.preview_toast_style);
}

function set_mobile_portrait_style() {

    console.log(isPortrait);

    if (!isPortrait) return;

    mobile_landscope_style = get_mobile_style();
...
function rearrange_preview_ui() {
    ...
    if (parseInt($('.quiz_show .type_id').html()) < 12) {
        question_label = 'Question ' + questionId;
    }

    if (parseInt($('.quiz_show .type_id').html()) == 12 || parseInt($('.quiz_show .type_id').html()) == 13 || parseInt($('.quiz_show .type_id').html()) == 16) {
        question_label = 'Introduction';
    }

    if (parseInt($('.quiz_show .type_id').html()) == 14 || parseInt($('.quiz_show .type_id').html()) == 15) {
...
function preview(element) {
    ...
    switch ($('#submit_btn').html()) {
        case 'Submit':
            if ($('.quiz_show').find('.type_id').html() == '16') {
                ...
                if (!$('#user_info')[0].checkValidity()) {
                    show_modal('error', 'Warning', 'You must complete the form correctly to continue.');
                } else {
                    $('#submit_btn').html('Continue');
        ...
        case 'Continue':
            ...
            if (parseInt($('.quiz_show').find('.type_id').html()) < 12) questionId++;
